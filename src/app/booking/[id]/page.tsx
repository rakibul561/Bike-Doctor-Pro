...
const Booking: React.FC<{ params: { id: string } }> = ({ params }) => {
  ...
  // Initialize product with the correct type
  const [product, setProduct] = useState<Product | null>(null);

  // Memoize the loadProducts function
  const loadProducts = useCallback(async () => {
    const detailse = await getProductDetails(params.id);
    setProduct(detailse.product);
  }, [params.id]);

  // Destructure product safely
  const { img, _id, name, price } = product || {}; // Default to empty object if product is null
